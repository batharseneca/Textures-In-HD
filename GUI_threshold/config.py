...
class Introduction:
	...
	def makeTemplate(self):
		...
		self.infoFrame.pack(side="top" ,expand=1 ,fill="x" ,pady=10)
		infoLabel = tk.Text(self.infoFrame,bg=self.myParent.cget("bg"), font=font, height=18,width=110, wrap="word",bd=0)
...
'''.strip())
		...
		threshCheckBox = tk.Checkbutton(self.threshFrame, text="Enable", variable=self.threshCheck, command=self.threshFunction, padx=70)
		threshCheckBox.pack(side="left",expand=1, fill="none", anchor="w")
		...
		textureCheckBox = tk.Checkbutton(self.textureFrame, text="Enable", variable=self.textureCheck, command=self.textureFunction, padx=70)
		textureCheckBox.pack(side="left",expand=1, fill="none", anchor="w")
		...
		self.runButton = tk.Button(self.runFrame, text="Run!")
	...
	def bitFunction(self,event=None):
		...
		if(self.bitCheck.get() == 1):
			self.bitImage.pack(in_=self.bitImageFrame)
			self.config.bitConversion=True
		else:
			self.bitImage.pack_forget()
			self.config.bitConversion=False
